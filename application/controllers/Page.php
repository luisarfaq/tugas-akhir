...
class Page extends CI_Controller
{
	...
	function tracking_status()
	{
		if (isset($_POST['submit1']) or $this->uri->segment(3) != '') {
			...
			$data_transaksi = $this->db->where('kode_transaksi',$kode_transaksi)->get('tb_toko_penjualan')->row();

			$status_midtrans=@get_notif($kode_transaksi);
			if ($status_midtrans->transaction_status=='settlement' && $data_transaksi->proses==0) {
				$this->db->where('kode_transaksi',$kode_transaksi)->update('tb_toko_penjualan',['proses'=>2]);
			}
